...
namespace PdfiumViewer
{
    public abstract class PanningZoomingScrollControl : CustomScrollControl
    {
        ...
        public const double DefaultZoomMax = 15;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (isDragging)
            {
                // Set con trỏ thành cross
                Cursor.Current = Cursors.Cross;
                // Xác định vùng hình chữ nhật khi kéo chuột
                ...
                // Gọi sự kiện SelectionChanged để thông báo vùng chọn
                SelectionChanged?.Invoke(selectionRect);
                // Vẽ vùng chụp (tạm thời)
                Invalidate(); // Làm mới để vẽ vùng
            }
            else
            {
        ...
        private void CaptureScreen(Rectangle rect)
        {
            using (Bitmap bitmap = new Bitmap(rect.Width, rect.Height))
            {
                ...
                {
                    // Chụp nội dung màn hình trong vùng chỉ định
                    g.CopyFromScreen(
                        this.PointToScreen(rect.Location),
                        Point.Empty,
                        rect.Size
                    );
                }
                // Kích hoạt sự kiện để gửi ảnh
                ScreenshotCaptured?.Invoke((Bitmap)bitmap.Clone());
                // Lưu bitmap hoặc xử lý theo nhu cầu
                bitmap.Save("screenshot.png", System.Drawing.Imaging.ImageFormat.Png);
                MessageBox.Show("Chọn vùng thành công!");
            }
        }
